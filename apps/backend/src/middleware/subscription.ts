...
import { getCache, setCache, CACHE_TTL, CACHE_KEYS } from '../utils/cache.js'
...
// Use cache key from centralized cache utility
const SUBSCRIPTION_CACHE_KEY = CACHE_KEYS.subscription
...
export async function getSubscription(userId: string) {
  ...
  const cached = await getCache<CachedSubscription>(SUBSCRIPTION_CACHE_KEY(userId))
  if (cached) {
    return cached
  }
  ...
  // Cache for 5 minutes
  await setCache(SUBSCRIPTION_CACHE_KEY(userId), result, CACHE_TTL.MEDIUM)
 ...
 */
function getEffectiveTier(subscription: CachedSubscription): SubscriptionTier {
...
export async function invalidateSubscriptionCache(userId: string): Promise<void> {
  ...
  await deleteCache(SUBSCRIPTION_CACHE_KEY(userId))
}
