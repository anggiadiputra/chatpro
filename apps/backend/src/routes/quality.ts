    ...
    for (const rating of allRatings) {
      if (!phoneNumbersMap.has(rating.phoneNumberId)) {
        phoneNumbersMap.set(rating.phoneNumberId, {
          ...
          displayPhoneNumber: user.phoneNumberId === rating.phoneNumberId 
            ? 'Primary Phone' 
            : rating.phoneNumberId,
...
app.post('/sync', requireRole(['ADMIN', 'BUSINESS_OWNER']), async (c: Context) => {
  try {
    ...
    if (!user.phoneNumberId) {
      return c.json({ error: { code: 'BadRequest', message: 'No phone number configured' } }, 400)
    }
    ...
    let qualityData
    try {
      qualityData = await whatsapp.getQualityRating(user.phoneNumberId)
    } catch (metaError: any) {
